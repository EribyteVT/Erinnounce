...
pipeline {
    ...
    environment {
        DOCKER_IMAGE_NAME = "eribyteofficial/erinnounce"
    }
    ...
    stages {
        stage("Checkout from github repo"){
            steps{
                ...
                git  branch: "${params.BRANCH}", url: 'https://github.com/EribyteVT/Erinnounce.git'
                script{
            ...
            steps {
                ...
                withCredentials([string(credentialsId: 'CA_CERTIFICATE', variable: 'cert'),
                    ...
                    kubeconfig(caCertificate: "${cert}", credentialsId: "${cred}", serverUrl: "${url}"){
                        ...
                        sh "kubectl rollout restart deployment erinnounce-$environ"
                    }
                }
            }
        }
